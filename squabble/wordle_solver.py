...
class WordleSolver(Solver):
    ...
    def guess(self, guess_count):
        if guess_count == 0:
            ...
            # print(self.last_word, self.solution)
    ...
    def get_best_word(self, attempting_solve=True):
        ...
        for solution in self.solutions:
            guess_count = Counter(len(candidates))
            solution_timer = Timer()
            solution_count()
            for candidate in candidates:
                ctimer = Timer()
                guess_count()
                rs = self.get_remaining_solutions(candidate, self.get_code(candidate, solution))
                ...
                c_average += ctimer()
                print(f"Solutions: {solution_count} {s_average():.6f} | Guesses: {guess_count} {c_average():.6f}", end='\r')
            s_average += solution_timer()
        best_word = self.sort_keys(scores, False)[0]
        best_word = self.break_tie(list(scores.items()))
        print(f"Guess took {guess_timer} second(s)")
        return best_word
